...
class StackPtrNet(nn.Module):
    ...
    def _get_encoder_output(self, input_word, input_char, input_pos, mask_e=None, length_e=None, hx=None):
        ...
        if self.char:
            ...
            # Debugging: Print the size of 'char' before reshaping
            print("Original shape of char:", char_size)
            
            # First transform to [batch * length, char_length, char_dim]
            # Then transpose to [batch * length, char_dim, char_length]
            char = char.view(char_size[0] * char_size[1], char_size[2], char_size[3]).transpose(1, 2)
            
            # Debugging: Print the size of 'char' after reshaping and transposing
            print("Shape of char after view and transpose:", char.size())
            
            # Check for NaN or Inf values
            print("Contains NaN values in char:", torch.isnan(char).any())
            print("Contains Infinity values in char:", torch.isinf(char).any())
            
            # Put into CNN [batch*length, char_filters, char_length]
            # Then apply max pooling [batch * length, char_filters]
            char, _ = self.conv1d(char).max(dim=2)
            
            # Debugging: Print the size of 'char' after conv1d and max pooling
            print("Shape of char after conv1d and max pooling:", char.size())
            ...
            # Debugging: Print the size of 'src_encoding' after concatenation
            print("Shape of src_encoding after concatenation with char:", src_encoding.size())
        ...
        if self.pos:
            ...
            # Debugging: Print the size of 'src_encoding' after concatenation with pos
            print("Shape of src_encoding after concatenation with pos:", src_encoding.size())
        ...
        # Debugging: Print the size of 'output'
        print("Shape of output after dropout:", output.size())
    ...
    def loss(self, input_word, input_char, input_pos, heads, stacked_heads, children, siblings, stacked_types, label_smooth,
        ...
        # Print shapes of outputs for debugging (Python 2.7 print statement)
        print('Shape of output_enc:', output_enc.size())
        print('Shape of hn:', hn[0].size() if hn is not None else 'None')
        ...
        print('Shape of output_dec:', output_dec.size())
        ...
        # Print shapes of arc and type tensors for debugging
        print('Shape of arc_h:', arc_h.size())
        print('Shape of arc_c:', arc_c.size())
        print('Shape of type_h:', type_h.size())
        print('Shape of type_c:', type_c.size())
        
        # [batch, length_decoder, length_encoder]
        out_arc = self.attention(arc_h, arc_c, mask_d=mask_d, mask_e=mask_e).squeeze(dim=1)
        
        # Print shape of out_arc for debugging
        print('Shape of out_arc:', out_arc.size())
        ...
        # Print shape of out_type for debugging
        print('Shape of out_type:', out_type.size())
        ...
        # Print shapes of loss_arc and loss_type for debugging
        print('Shape of loss_arc:', loss_arc.size())
        print('Shape of loss_type:', loss_type.size())
        
            # Compute coverage loss
        # [batch, length_decoder, length_encoder]
        coverage = torch.exp(loss_arc).cumsum(dim=1)
        
        # Print shape of coverage for debugging
        print('Shape of coverage:', coverage.size())
