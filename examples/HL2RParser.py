...
def main():
    ...
    args_parser.add_argument('--word_embedding', choices=['glove', 'senna', 'sskip', 'polyglot', 'word2vec'], help='Embedding for words', required=True)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in range(1, int(num_batches + 1)):
            ...
            optim.zero_grad()
            loss_arc, loss_type, loss_cov, num = network.loss(
                word, char, pos, heads, stacked_heads, children, sibling, stacked_types, 
                label_smooth, skip_connect=skip_connect, mask_e=masks_e, length_e=lengths_e, 
        ...
        # Evaluate performance on the dev set for each epoch
        with torch.no_grad():
            ...
            dev_ucorr, dev_lcorr, dev_total = 0.0, 0.0, 0
            dev_ucomlpete, dev_lcomplete = 0.0, 0.0
            dev_ucorr_nopunc, dev_lcorr_nopunc, dev_total_nopunc = 0.0, 0.0, 0
            dev_ucomlpete_nopunc, dev_lcomplete_nopunc = 0.0, 0.0
            dev_root_corr, dev_total_root = 0.0, 0.0
            dev_total_inst = 0.0
            ...
            for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_dev, batch_size):
                ...
                heads_pred, types_pred, _, _ = network.decode(
                    word, char, pos, mask=masks, length=lengths, beam=beam,
                ...
                stats, stats_nopunc, stats_root, num_inst = parser.eval(
                    word, pos, heads_pred, types_pred, heads, types,
            ...
            # Save the best model and evaluate on the test set if necessary
